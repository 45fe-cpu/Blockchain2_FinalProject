...
contract GovTokenTest is BaseSetup {
    ...
    function test_initialSupply() public view {
        assertGt(govToken.totalSupply(), 0);
    }

    function test_deployerBalance() public view {
        assertGt(govToken.balanceOf(deployer), 0);
    }

    function test_mint() public {
        vm.prank(deployer);
        govToken.mint(alice, 100e18);
        assertGe(govToken.balanceOf(alice), 100e18);
    }
    ...
    function test_delegate() public view {
    ...
    function test_transfer() public {
        vm.prank(alice);
        govToken.transfer(bob, 100e18);
    }

    function test_votingPower() public view {
        assertGt(govToken.getVotes(deployer), 0);
    }
}

contract IronTokenTest is BaseSetup {
    function test_name() public view {
        assertEq(ironToken.name(), "IronToken");
    }

    function test_symbol() public view {
        assertEq(ironToken.symbol(), "IRON");
    }

    function test_mint() public {
        vm.prank(deployer);
        ironToken.mint(alice, 50e18);
    }
    ...
    function test_burn() public {
        vm.prank(alice);
        ironToken.burn(10e18);
    }

    function test_burnExceedsBalance() public {
        vm.prank(alice);
        vm.expectRevert();
        ironToken.burn(999_999e18);
    }
}
...
contract GameItemsTest is BaseSetup {
    ...
    function test_craftingFee() public view {
        assertEq(gameItems.craftingFee(), 10e18);
    }
