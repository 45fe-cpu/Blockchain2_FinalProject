...
const ABIs = {
    ...
    GameEngine: [
        "function farmLoot(uint256 loops)"
    ],
    GameItems: [
...
function initContracts() {
    ...
    if (CONTRACT_ADDRESSES.IronToken.startsWith("0x...")) {
        console.warn("Contract addresses not set. Please update CONTRACT_ADDRESSES.");
        return;
    }

    contracts.ironToken = new ethers.Contract(CONTRACT_ADDRESSES.IronToken, ABIs.ERC20, signer);
    contracts.govToken = new ethers.Contract(CONTRACT_ADDRESSES.GovToken, ABIs.ERC20, signer);
    contracts.gameEngine = new ethers.Contract(CONTRACT_ADDRESSES.GameEngine, ABIs.GameEngine, signer);
    contracts.gameItems = new ethers.Contract(CONTRACT_ADDRESSES.GameItems, ABIs.GameItems, signer);
    contracts.vault = new ethers.Contract(CONTRACT_ADDRESSES.IronVault, ABIs.IronVault, signer);
    contracts.amm = new ethers.Contract(CONTRACT_ADDRESSES.AMM, ABIs.AMM, signer);
    contracts.shop = new ethers.Contract(CONTRACT_ADDRESSES.IronShop, ABIs.IronShop, signer);
    contracts.oracle = new ethers.Contract(CONTRACT_ADDRESSES.Oracle, ABIs.Oracle, provider);
    contracts.governor = new ethers.Contract(CONTRACT_ADDRESSES.Governor, ABIs.Governor, signer);
...
async function loadUserData() {
    ...
    try {
        ...
        document.getElementById("govBalance").innerText = ethers.utils.formatEther(govBal);

        // Header balance
        document.getElementById("headerIronBalance").innerText = `${parseFloat(ironBalFormatted).toFixed(2)} IRON`;
        ...
        document.getElementById("stakedIron").innerText = ethers.utils.formatEther(vaultShares);
        document.getElementById("vaultTotalAssets").innerText = ethers.utils.formatEther(totalAssets);

        // Governance
        const votes = await contracts.govToken.getVotes(userAddress);
        document.getElementById("votingPower").innerText = ethers.utils.formatEther(votes);
...
function renderInventory(partA, partB, sword) {
    ...
    const items = [
        { count: partA, name: "Grip", img: "bafkreiaheg5mug44s7yon5euap5yhrknsitotfvxpjnuimgs4hm3637ape", class: "" },
        { count: partB, name: "Blade", img: "bafkreidyhdg6v5oby5jutg5aka56se3iioxfh7u5rnip6minbd5girkl6u", class: "" },
        { count: sword, name: "Legendary Sword", img: "bafkreies4xxqnpg73aoqlcnjk4xr2hwapvnus4ertur76meq6a6t3yzae4", class: "legendary" }
    ];
    ...
    items.forEach(item => {
        for (let i = 0; i < item.count; i++) {
            ...
            card.innerHTML = `
                <div class="item-icon">
                    <img src="https://ipfs.io/ipfs/${item.img}" alt="${item.name}" style="max-height: 80px; border-radius: 8px;">
                </div>
                <h3>${item.name}</h3>
                <p style="font-size: 0.8em; color: var(--text-muted); margin-top: 5px;">Unit #${i + 1}</p>
            `;
...
const STATE_STRINGS = ["Pending", "Active", "Canceled", "Defeated", "Succeeded", "Queued", "Expired", "Executed"];

async function loadProposals() {
    try {
        const container = document.getElementById("proposalsContainer");
        container.innerHTML = '<p style="text-align: center;">Loading proposals...</p>';
        ...
        if (events.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: var(--text-muted);">No proposals found.</p>';
            return;
        }
        ...
        for (let i = events.length - 1; i >= 0; i--) {
            ...
            card.innerHTML = `
                <div class="proposal-header">
                    <h4 style="word-break: break-all;">Proposal ID: ${proposalId.toString().substring(0, 10)}...</h4>
                    <span class="status badge ${stateStr === 'Active' ? 'active' : ''}">${stateStr}</span>
                </div>
                <p><strong>Description:</strong> ${description}</p>
                <div style="font-size: 0.9em; margin-bottom: 10px;">
                    <span style="color: green;">For: ${ethers.utils.formatEther(votes.forVotes)}</span> | 
                    <span style="color: red;">Against: ${ethers.utils.formatEther(votes.againstVotes)}</span>
                </div>
                <div class="action-group">
                    <button class="btn secondary-btn" onclick="voteProposal('${proposalId.toString()}', 1)" ${state !== 1 ? 'disabled' : ''}>Vote For</button>
                    <button class="btn danger-btn" onclick="voteProposal('${proposalId.toString()}', 0)" ${state !== 1 ? 'disabled' : ''}>Vote Against</button>
                </div>
                <div class="action-group mt-2">
                    <button class="btn secondary-btn" onclick="queueProposal('${proposalId.toString()}', '${description}')" ${state !== 4 ? 'disabled' : ''}>Queue (Succeeded)</button>
                    <button class="btn primary-btn glow-effect" onclick="executeProposal('${proposalId.toString()}', '${description}')" ${state !== 5 ? 'disabled' : ''}>Execute (Queued)</button>
                </div>
            `;
            container.appendChild(card);
        }
...
window.voteProposal = async function (proposalIdStr, support) {
    try {
        const tx = await contracts.governor.castVote(ethers.BigNumber.from(proposalIdStr), support);
        await tx.wait();
...
window.queueProposal = async function(proposalIdStr, description) {
    try {
        const { targets, values, calldatas, descriptionHash } = getProposalDataFromDescription(description);
...
function bindContractActions() {
    ...
    document.getElementById("swapBtn").addEventListener("click", async () => {
        ...
        try {
            const tokenInContract = tokenInSymbol === "GOV" ? contracts.govToken : contracts.ironToken;

            // Approve
            const approveTx = await tokenInContract.approve(CONTRACT_ADDRESSES.AMM, amount);
    ...
    document.getElementById("approveCraftBtn").addEventListener("click", async () => {
        try {
            const tx = await contracts.ironToken.approve(CONTRACT_ADDRESSES.GameItems, ethers.constants.MaxUint256);
            await tx.wait();
    ...
    document.getElementById("approveVaultBtn").addEventListener("click", async () => {
        try {
            const amount = ethers.utils.parseEther(document.getElementById("depositAmount").value || "0");
            const tx = await contracts.ironToken.approve(CONTRACT_ADDRESSES.IronVault, amount);
            await tx.wait();
    ...
    document.getElementById("depositVaultBtn").addEventListener("click", async () => {
        try {
            const amount = ethers.utils.parseEther(document.getElementById("depositAmount").value || "0");
    ...
    document.getElementById("withdrawVaultBtn").addEventListener("click", async () => {
        try {
            const shares = ethers.utils.parseEther(document.getElementById("withdrawAmount").value || "0");
            const tx = await contracts.vault.withdraw(shares, userAddress, userAddress);
            await tx.wait();
    ...
    document.getElementById("refreshProposalsBtn").addEventListener("click", async () => {
        await loadProposals();
    });
}
