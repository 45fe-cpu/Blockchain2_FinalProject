...
contract VerifyDeployment is Script {
    function run() external {
        ...
        address timelockAddr = stdJson.readAddress(json, "$.transactions[?(@.contractName == 'Timelock' && @.transactionType == 'CREATE')].contractAddress");
        address governorAddr = stdJson.readAddress(json, "$.transactions[?(@.contractName == 'MyGovernor' && @.transactionType == 'CREATE')].contractAddress");
        address engineAddr = stdJson.readAddress(json, "$.transactions[?(@.contractName == 'ERC1967Proxy' && @.transactionType == 'CREATE')].contractAddress");
        address ironTokenAddr = stdJson.readAddress(json, "$.transactions[?(@.contractName == 'IronToken' && @.transactionType == 'CREATE')].contractAddress");
        address gameItemsAddr = stdJson.readAddress(json, "$.transactions[?(@.contractName == 'GameItems' && @.transactionType == 'CREATE')].contractAddress");
